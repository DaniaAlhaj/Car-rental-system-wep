<?php
session_start();

try {
   include "dbconfig.php";

    $stmt = $pdo->prepare('INSERT INTO user (userid, username, dateofBirth, idnumber, 
     email, phone, user_password) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $_SESSION['userid'],
        $_SESSION['username'],
        $_SESSION['dateofBirth'],
        $_SESSION['idnumber'],
        $_SESSION['email'],
        $_SESSION['phone'],
        $_SESSION['user_password']
    ]);

    $stmt = $pdo->prepare('INSERT INTO credit_cards ( userid, card_number, expiry_date, 
     card_name, bank_name) VALUES ( ?, ?, ?, ?, ?)');
 $stmt->execute([
    $_SESSION['userid'],
    $_SESSION['card_number'],
    $_SESSION['expiry_date'],
    $_SESSION['card_name'],
    $_SESSION['bank_name']
]);
   $stmt = $pdo->prepare('INSERT INTO addresses ( userid, flat, street, 
     city, country) VALUES ( ?, ?, ?, ?, ?)');
 $stmt->execute([
    $_SESSION['userid'],
    $_SESSION['flat'],
    $_SESSION['street'],
    $_SESSION['city'],
    $_SESSION['country']
]);
$_SESSION['regester']=true;




?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Car Rental Search</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
  
<header class="header-container">
    <figure>
        <img src="img/logob.png" alt="Agency Logo">
    </figure>
    <p><strong>Birzeit Car Rental Agency</strong></p>

    <div class="links_header">
        <ul> 
        <li><a href="about_us_page.php">About Us</a></li>
            <li><a href="profile.php">User Profile</a></li>
            <li><a href="basket.php">Shopping Basket</a></li>
            <li><a href="register.php">Register</a></li>
            <li><a href="login.php">Login</a></li>
            <li><a href="logout.php">Logout</a></li>

        </ul>
    </div>
</header>

<main class="main_container">
    <nav class="main-nav">
        <ul> 
        <li><a href="usersearch.php">Search car</a></li>
            <li><a href="viewRentedCar.php">View rented cars</a></li>
            <li><a href="user_return.php">Return a car</a></li>
        </ul>
    </nav>

    <div class="main-content">
    <p>Registration successful. Your customer ID is: <?php $_SESSION['userid'] ?> </p> 
         <a href='login.php'>Click here to login</a>
    </div>
</main>

<footer class="footer_container">
    <figure>
        <img src="img/logob.png" alt="BirzeitCarRentalAgency logo">
    </figure>
    <p><strong>Birzeit Car Rental Agency</strong></p>
    <address class="address_footer">
        <p>Contact us at:</p>
        <p><em>Email:</em> <a href="mailto:BirzeitCarRentalAgency@gmail.com"><em>BirzeitCarRentalAgency@gmail.com</em></a></p>
        <p><em>Telephone:</em> <a href="tel:0569699377"><em>0569699377</em></a></p>
    </address>
    <a href="contact.html"><em>Contact Us</em></a>
</footer> 
</body>
</html>

<?php

} catch (PDOException $e) {
    echo "Error: " . $e->getMessage();
}
   
    
?>
